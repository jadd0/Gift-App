...
export async function POST({ request, fetch, cookies }) {
	...
	const moderator = await RemoveMember.removeMember(req.circleUUID, auth.uuid, auth.uuid)

	if (!moderator) {
		throw error(500, 'There has been an error whilst leaving this. Please try again later, and if this issue persists please email me at jaddalkwork@gmail.com')
	}
